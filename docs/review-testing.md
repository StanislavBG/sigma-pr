...
- **Без самопотвърждаващи се тестове** — асъртвайте конкретен литерал (`expect(label).toBe('123
  456,78 лв.')`), не стойност, предеривана през същата функция, която тества
  (`expect(label).toBe(money(x))`); последното минава и при счупена `money()`.
- **Пазете се от schema drift** — сверявайте `pragma_table_info` срещу истинската миграция, не
  срещу inline копие на `CREATE TABLE` в теста; копието мълчаливо изостава от следваща миграция и
  тестът продължава да минава върху грешна схема.
- **Partition/exhaustiveness инвариант, не „не е default bucket-а"** — за switch/case класификация
  тествайте, че всеки вход попада в точно един бъкет (сборът на бройки по бъкети = общ брой), не
  само че конкретен ред не е паднал в default-а — второто пропуска нов необработен случай, който
  тихо пада в грешен, но не default бъкет.
- **По един тест на cache-key параметър (CWE-349)** — всеки параметър, участващ в cache ключа
  (locale, filter, page), получава собствен тест, който доказва, че различна стойност дава
  различен ключ; един комбиниран happy-path тест пропуска ключова колизия между два различни
  входа.
- **Reproduce-the-bug-first за поправки** — поправка на коректностен бъг започва с червен тест,
  който точно възпроизвежда счупеното поведение (red), после production кодът (green); без тест
  „доказва" реда на действията.
